...
from pet_monitor.common import (DATA_DIR, NetworkInterfaceInfo,
                                get_db_connection)
from pet_monitor.settings import RateLimiter, get_settings

_logger = logging.getLogger(__name__)


SCHEMA_SQL = '''\
CREATE TABLE snmp_ip_map_results (
...
class SNMPResult(NamedTuple):
    ...
    def is_duplicate(self, other: 'SNMPResult') -> bool:
        def _match(a: Optional[str], b: Optional[str]) -> bool:
                    return a is not None and a == b
        return _match(self.ip, other.ip) or _match(self.mac, other.mac)

    @staticmethod
    def filter_duplicates(vals: Iterable['SNMPResult']) -> set['SNMPResult']:
        results = set()
        for v1 in vals:
...
class SNMPScraper():
    def __init__(self, settings: SNMPSettings) -> None:
        ...
        self.conn = get_db_connection(DATA_DIR / 'snmp.sqlite3', SCHEMA_SQL)

    def get_all_results(self) -> set[SNMPResult]:
        cur = self.conn.cursor()
        cur.execute("SELECT timestamp, ip, mac, host_name FROM snmp_ip_map_results;""")
        return {SNMPResult(*r) for r in cur.fetchall()}

    def _check_for_scan_complete(self):
        #     {'snmp': {'command_line': 'snmp -oX - -sn 192.168.1.100-255',
        #       'scaninfo': {},
        ...
        #                            'status': {'state': 'up',
        #                                       'reason': 'arp-response'}},
        if self.snmp_interface.result is not None:
            if 'snmp' in self.snmp_interface.result:
                _logger.debug(self.snmp_interface.result['snmp'])

            if 'scan' in self.snmp_interface.result:
                cur_results = self.get_all_results()
                scan: PortScannerHostDict = self.snmp_interface.result['scan']  # type: ignore
                timestamp = int(time.time())
                new_results: set[SNMPResult] = set()
                for ip, result in scan.items():
                    ...
                    result = SNMPResult(timestamp, ip, mac, host_name)

                    # Apparently, SNMP can sometimes return results with duplicate MAC addresses in a single scan?
                    is_duplicate = False
                    ...
                    if matches > 1:
                        cur.execute('DELETE FROM snmp_ip_map_results WHERE ip=? OR mac=? OR host_name=? LIMIT ?;',
                                    result[1:] + (matches - 1,))
                        matches = 1
                    ...
                    try:
                        # INSERT
                        if matches == 0:
                            cur.execute('INSERT INTO snmp_ip_map_results(timestamp, ip, mac, host_name) VALUES (?, ?, ?, ?);', result)
                        # UPDATE
                        else:
                            cur.execute(
                                'UPDATE snmp_ip_map_results SET timestamp=?, ip=?, mac=?, host_name=? WHERE ip=? OR mac=? OR host_name=? LIMIT 1;',
                                result + result[1:])
                    except:
                        print(cur_results)
                        print(new_results)
            ...
            self.snmp_interface.result = None

    def is_ready(self):
        return self.rate_limiter.is_ready() or self.snmp_interface.result is not None
    ...
    def update(self):
        ...
        if self.snmp_interface.in_progress:
            _logger.error('Attemtping new scan while previous run has not completed.')
            return

        self.snmp_interface.scan_ranges()


def main():
    logging.basicConfig(level=logging.DEBUG)

    settings = get_settings().snmp_settings
    if settings is None:
        print("SNMP settings not found.")
        return
    scraper = SNMPScraper(settings)
